...
Tick.prototype = {
	...
	getLabelPosition: function (x, y, label, horiz, labelOptions, tickmarkOffset, index, step) {
		var axis = this.axis,
			...
			baseline = axis.flot_chart.renderer.fontMetrics(labelOptions.style.fontSize).b,
...
PlotLineOrBand.prototype = {
	...
	render: function () {
		var plotLine = this,
			...
			attribs,
			renderer = axis.flot_chart.renderer;
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	var inverted = axis.flot_chart.inverted;
...
StackItem.prototype = {
	...
	render: function (group) {
		...
		// Create new label
			...
			this.label =
				this.axis.flot_chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
...
Axis.prototype = {
	...
	update: function (newOptions, redraw) {
		...
		newOptions = chart.options[this.xOrY + 'Axis'][this.options.old_index] = merge(this.userOptions, newOptions);
	...
	remove: function (redraw) {
		...
		chart.options[key].splice(this.options.old_index, 1);
	...
	getSeriesExtremes: function () {
		...
		each(axis.series, function (series) {
			...
			if (series.visible || !chart.options.flot_chart.ignoreHiddenSeries) {
...
Pointer.prototype = {
	...
	init: function (chart, options) {
		...
		var chartOptions = options.flot_chart,
	...
	drag: function (e) {
		...
		var chart = this.chart,
			chartOptions = chart.options.flot_chart,
			chartX = e.chartX,
			chartY = e.chartY,
...
Chart.prototype = {
	...
	initSeries: function (options) {
		var chart = this,
			optionsChart = chart.options.flot_chart,
	...
	adjustTickAmounts: function () {
		if (this.options.flot_chart.alignTicks !== false) {
	...
	getStacks: function () {
		...
		each(chart.series, function (series) {
			if (series.options.stacking && (series.visible === true || chart.options.flot_chart.ignoreHiddenSeries === false)) {
	...
	showResetZoom: function () {
		var chart = this,
			lang = defaultOptions.lang,
			btnOptions = chart.options.flot_chart.resetZoomButton,
			theme = btnOptions.theme,
			states = theme.states,
			alignTo = btnOptions.relativeTo === 'flot_chart.html' ? null : 'plotBox';
	...
	zoom: function (event) {
		...
		if (hasZoomed) {
			chart.redraw(
				pick(chart.options.flot_chart.animation, event && event.animation, chart.pointCount < 100) // animation
			);
		}
	},
		...
		each(chartSeries, function (series) {
			...
			if (isString(linkedTo)) {
				if (linkedTo === ':previous') {
					linkedTo = chart.series[series.old_index - 1];
				} else {
...
Point.prototype = {
	...
	init: function (series, options, x) {
		...
		if (series.options.colorByPoint) {
			colors = series.options.colors || series.flot_chart.options.colors;
	 ...
	 */
	onMouseOut: function () {
		var chart = this.series.flot_chart,
			hoverPoints = chart.hoverPoints;
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		stableSort(chartSeries, function (a, b) {
			return pick(a.options.old_index, a._i) - pick(b.options.old_index, a._i);
		});
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.old_index) ||
							(seriesOptions[AXIS] !== UNDEFINED && seriesOptions[AXIS] === axisOptions.id) ||
							(seriesOptions[AXIS] === UNDEFINED && axisOptions.old_index === 0)) {
	...
	setStackedPoints: function () {
		if (!this.options.stacking || (this.visible !== true && this.chart.options.flot_chart.ignoreHiddenSeries !== false)) {
			return;
		}
		...
		for (i = 0; i < yDataLength; i++) {
			...
			stack = stacks[key][x];
			stack.points[series.old_index] = [stack.cum || 0];
			...
			stack.points[series.old_index].push(stack.cum);
	...
	setPercentStacks: function () {
		...
		each([stackKey, '-' + stackKey], function (key) {
			...
			while (i--) {
				...
				pointExtremes = stack && stack.points[series.old_index];
				if (pointExtremes) {
	...
	translate: function () {
		...
		for (i = 0; i < dataLength; i++) {
			...
			if (stacking && series.visible && stack && stack[xValue]) {
				...
				stackValues = pointStack.points[series.old_index];
	...
	setVisible: function (vis, redraw) {
		var series = this,
			...
			showOrHide,
			ignoreHiddenSeries = chart.options.flot_chart.ignoreHiddenSeries,
				...
				each(this.points, function (point) {
					...
					if (dataLabel && dataLabel._pos) {
						...
						if (connector) {
							...
							point.connector = connector = series.flot_chart.renderer.path(connectorPath).attr({
