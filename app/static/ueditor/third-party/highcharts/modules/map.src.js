...
(function (Highcharts) {
	...
	wrap(Axis.prototype, 'setAxisTranslation', function (proceed) {
		...
		if (chart.options.flot_chart.type === 'map' && !isXAxis && xAxis.transA !== UNDEFINED) {
	...
	extend(Chart.prototype, {
		...
		mapZoom: function (howMuch, centerXArg, centerYArg) {
			...
			var chart = this,
				...
				animation = pick(chart.options.flot_chart.animation, true),
				delay,
	...
	seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
		...
		drawLegendSymbolGradient: function (legend, item) {
			var spacing = legend.options.symbolPadding,
				...
				positionY,
				positionX,
				gradientSize = this.flot_chart.renderer.fontMetrics(legend.options.itemStyle.fontSize).h,
			...
			item.fromText = this.flot_chart.renderer.text(
					item.fromLabel,	// Text.
			...
			item.legendSymbol = this.flot_chart.renderer.rect(
			...
			item.toText = this.flot_chart.renderer.text(
					item.toLabel,
		...
		animateDrilldown: function (init) {
			var toBox = this.flot_chart.plotBox,
				level = this.flot_chart.drilldownLevels[this.flot_chart.drilldownLevels.length - 1],
				fromBox = level.bBox,
				animationOptions = this.flot_chart.options.drilldown.animation,
				scale;
